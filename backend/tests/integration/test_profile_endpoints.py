"""Integration tests for profile endpoints."""
import pytest
from datetime import datetime, timezone


# Sample test data
SAMPLE_USER_ID = "test-user-123"
SAMPLE_USER = {
    "id": SAMPLE_USER_ID,
    "email": "test@example.com",
    "role": "authenticated"
}
SAMPLE_PROFILE = {
    "id": SAMPLE_USER_ID,
    "email": "test@example.com",
    "full_name": "John Doe",
    "date_of_birth": "1980-01-15",
    "created_at": datetime.now(timezone.utc).isoformat(),
    "updated_at": datetime.now(timezone.utc).isoformat()
}


def test_get_profile_unauthorized(client):
    """Test that getting profile requires authentication."""
    response = client.get("/api/profile")
    assert response.status_code == 401


def test_get_profile_success(app, client, mock_db):
    """Test successfully getting profile."""
    from app.core.auth import get_current_user
    from app.core.dependencies import get_db

    async def override_get_current_user():
        return SAMPLE_USER

    async def override_get_db():
        return mock_db

    app.dependency_overrides[get_current_user] = override_get_current_user
    app.dependency_overrides[get_db] = override_get_db

    mock_db.query.return_value = [SAMPLE_PROFILE]

    response = client.get(
        "/api/profile",
        headers={"Authorization": "Bearer test-token"}
    )

    assert response.status_code == 200
    data = response.json()
    assert data["full_name"] == "John Doe"
    assert data["email"] == "test@example.com"


def test_get_profile_creates_if_not_exists(app, client, mock_db):
    """Test that getting profile creates it if it doesn't exist."""
    from app.core.auth import get_current_user
    from app.core.dependencies import get_db

    async def override_get_current_user():
        return SAMPLE_USER

    async def override_get_db():
        return mock_db

    app.dependency_overrides[get_current_user] = override_get_current_user
    app.dependency_overrides[get_db] = override_get_db

    # First query returns empty (profile doesn't exist)
    mock_db.query.return_value = []
    # Insert creates the profile
    new_profile = {**SAMPLE_PROFILE, "full_name": None}
    mock_db.insert.return_value = [new_profile]

    response = client.get(
        "/api/profile",
        headers={"Authorization": "Bearer test-token"}
    )

    assert response.status_code == 200
    # Verify insert was called
    mock_db.insert.assert_called_once()


def test_update_profile_unauthorized(client):
    """Test that updating profile requires authentication."""
    response = client.patch(
        "/api/profile",
        json={"full_name": "Jane Doe"}
    )
    assert response.status_code == 401


def test_update_profile_success(app, client, mock_db):
    """Test successfully updating profile."""
    from app.core.auth import get_current_user
    from app.core.dependencies import get_db

    async def override_get_current_user():
        return SAMPLE_USER

    async def override_get_db():
        return mock_db

    app.dependency_overrides[get_current_user] = override_get_current_user
    app.dependency_overrides[get_db] = override_get_db

    mock_db.query.return_value = [SAMPLE_PROFILE]
    updated_profile = {**SAMPLE_PROFILE, "full_name": "Jane Doe"}
    mock_db.update.return_value = updated_profile

    response = client.patch(
        "/api/profile",
        json={"full_name": "Jane Doe"},
        headers={"Authorization": "Bearer test-token"}
    )

    assert response.status_code == 200
    data = response.json()
    assert data["full_name"] == "Jane Doe"


def test_update_profile_creates_if_not_exists(app, client, mock_db):
    """Test that updating profile creates it if it doesn't exist."""
    from app.core.auth import get_current_user
    from app.core.dependencies import get_db

    async def override_get_current_user():
        return SAMPLE_USER

    async def override_get_db():
        return mock_db

    app.dependency_overrides[get_current_user] = override_get_current_user
    app.dependency_overrides[get_db] = override_get_db

    # First query returns empty (profile doesn't exist)
    mock_db.query.return_value = []
    # Insert creates the profile
    new_profile = {**SAMPLE_PROFILE, "full_name": None}
    mock_db.insert.return_value = [new_profile]
    # Update sets the name
    updated_profile = {**SAMPLE_PROFILE, "full_name": "Jane Doe"}
    mock_db.update.return_value = updated_profile

    response = client.patch(
        "/api/profile",
        json={"full_name": "Jane Doe"},
        headers={"Authorization": "Bearer test-token"}
    )

    assert response.status_code == 200
    data = response.json()
    assert data["full_name"] == "Jane Doe"
    # Verify insert was called to create profile
    mock_db.insert.assert_called_once()


def test_update_profile_no_changes(app, client, mock_db):
    """Test updating profile with no changes."""
    from app.core.auth import get_current_user
    from app.core.dependencies import get_db

    async def override_get_current_user():
        return SAMPLE_USER

    async def override_get_db():
        return mock_db

    app.dependency_overrides[get_current_user] = override_get_current_user
    app.dependency_overrides[get_db] = override_get_db

    mock_db.query.return_value = [SAMPLE_PROFILE]

    response = client.patch(
        "/api/profile",
        json={},
        headers={"Authorization": "Bearer test-token"}
    )

    assert response.status_code == 200
    data = response.json()
    # Should return current profile unchanged
    assert data["full_name"] == "John Doe"


def test_update_profile_with_date_of_birth(app, client, mock_db):
    """Test updating profile with date of birth."""
    from app.core.auth import get_current_user
    from app.core.dependencies import get_db

    async def override_get_current_user():
        return SAMPLE_USER

    async def override_get_db():
        return mock_db

    app.dependency_overrides[get_current_user] = override_get_current_user
    app.dependency_overrides[get_db] = override_get_db

    mock_db.query.return_value = [SAMPLE_PROFILE]
    updated_profile = {**SAMPLE_PROFILE, "date_of_birth": "1985-06-20"}
    mock_db.update.return_value = updated_profile

    response = client.patch(
        "/api/profile",
        json={"date_of_birth": "1985-06-20"},
        headers={"Authorization": "Bearer test-token"}
    )

    assert response.status_code == 200
    data = response.json()
    assert data["date_of_birth"] == "1985-06-20"
